...
WARMSTART_CKPT="/content/drive/MyDrive/EAP-and-Edge/warmstart_hap/warmstart_ckpt"

python src/prune/fpt2_ioi_hap.py \
    --report_to none \
    --do_train \
    --do_eval \
    ...
    --output_dir ./data/runs/ioi-hap-hard-${TAG}-elr${ELR}-llr${LLR}-relr${RELR}-rllr${RLLR}-es${EDGE_SPARSITY}-ns${NODE_SPARSITY}-t${TOTAL}/ \
