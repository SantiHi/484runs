...
from modeling_erazr import ErazrModelForSequenceTransformation as ErazrModelOriginal, ErazrTokenizer, ErazrConfig, get_config_weights_and_vocab
from modeling_erazr_compressed import ErazrModelForSequenceTransformation
...
class DataTrainingArguments:
    ...
    compression_dim: Optional[int] = field(
        default=0,
        metadata={"help": "Latent dimension k for factored subspace compression. 0 = disabled."}
    )
    compression_init: Optional[str] = field(
        default="gaussian",
        metadata={"help": "Projection initialization: 'gaussian', 'orthogonal', or path to pretrained weights."}
    )
    freeze_projections: Optional[bool] = field(
        default=True,
        metadata={"help": "Whether to freeze the compression projection matrices P and U."}
    )
...
def freeze_all_except_pruning_params(model):
    for n, p in model.named_parameters():
        ...
        elif 'compression_P' in n or 'compression_U' in n:
            pass  # respect requires_grad set during init (freeze_projections flag)
        else:
...
def main():
    ...
    model = ErazrModelForSequenceTransformation(
        config,
        compression_dim=data_args.compression_dim,
        compression_init=data_args.compression_init,
        freeze_projections=data_args.freeze_projections,
    ).to("cuda")
    ...
    # Reference model: use original (uncompressed) to provide d-dim writer states
    tracr_model = ErazrModelOriginal(config).to("cuda")
