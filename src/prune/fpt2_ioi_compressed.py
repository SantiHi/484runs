...
from modeling_fpt2_compressed import FPT2LMHeadModel
...
class DataTrainingArguments:
    ...
    compression_init: Optional[str] = field(
        default="gaussian",
        metadata={"help": "Projection initialization: 'gaussian' (JL sketching), 'orthogonal', or path to pretrained weights."}
    )
...
def freeze_all_except_pruning_params(model):
    for n, p in model.named_parameters():
        if 'log_alpha' in n or 'sparsity_lambda' in n:
            ...
            pass  # respect the requires_grad set during init (freeze_projections flag)
        else:
...
def main():
    ...
    # Reference model: use original (uncompressed) to provide d-dim logits and writer states
    gpt2_model = FPT2LMHeadModelOriginal.from_pretrained(
        "gpt2",
        with_embedding_nodes=data_args.with_embedding_nodes,
    ).to("cuda")
