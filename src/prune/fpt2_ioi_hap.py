...
def main():
    ...
    # HAP-hard: load gradient mask and register hooks to freeze restricted log_alphas
    import os as _hap_os
    hap_mask_path = _hap_os.environ.get(
        "HAP_MASK_PATH",
        "/content/drive/MyDrive/EAP-and-Edge/warmstart_hap/hap_mask.pt",
    )
    if _hap_os.path.exists(hap_mask_path):
        print(f"[HAP] loading gradient mask from {hap_mask_path}")
        hap_masks = torch.load(hap_mask_path)
        for _param_name, _mask in hap_masks.items():
            _param = model.get_parameter(_param_name)
            _device_mask = _mask.to(_param.device)
            ...
            _param.register_hook(_make_hook(_device_mask))
        _n_frozen = sum(m.sum().item() for m in hap_masks.values())
        print(f"[HAP] registered gradient hooks on {len(hap_masks)} parameters, "
              f"{_n_frozen} entries frozen")
    else:
        print(f"[HAP] WARNING: no mask file at {hap_mask_path}, training as plain warmstart")
