...
class FLlamaDecoderLayer(nn.Module):
    ...
    def attn_write(self, residual, x, corr_x=None, corr_x_d=None, P=None):
        ...
        if corr_src is not None:
            x = x + corr_src[self.attn_writer_idx : self.attn_writer_idx + self.num_heads] * (1-z)
    ...
    def mlp_write(self, residual, x, corr_x=None, corr_x_d=None, P=None):
        ...
        corr_src = corr_x_d if corr_x_d is not None else corr_x
        if corr_src is not None:
            x = x + corr_src[self.mlp_writer_idx] * (1-z)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # hidden_states is now (writers, batch_size, sequence_length, feat)
        ...
        residual, z_attn_nodes_sum = self.attn_write(
            residual, hidden_states, corr_x=corr_x, corr_x_d=corr_x_d, P=P,
        )

        # mlp_read: k-dim residual -> edge mask -> decompress to d-dim
        hidden_states, z_mlp_edges_sum = self.mlp_read(
            residual, corr_x=corr_x, embeds=embeds, U=U,
        )
...
class FLlamaModel(FLlamaPreTrainedModel):
    ...
    def write(self, tok_embeds, corr_x_d=None):
    ...
    def forward(
        ...
        # embed positions
        hidden_states, embeds, z_nodes_sum = self.write(inputs_embeds, corr_x_d=corr_x_d)
        z_edges_sum = 0

        # output_shape uses hidden_size (d) since read() decompresses back to d-dim
