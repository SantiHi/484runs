...
class CompressProjection(torch.autograd.Function):
    """d->k projection. Saves only P (k x d) for backward, not the d-dim input."""
    @staticmethod
    def forward(ctx, x, P):
...
class DecompressProjection(torch.autograd.Function):
    """k->d projection. Saves only U (d x k) for backward."""
    @staticmethod
    def forward(ctx, x, U):
...
class ErazrAttention(nn.Module):
    ...
    def read(self, x, corr_x=None, U=None):
        ...
        if corr_x is not None:
            corr_q = torch.einsum("wbsd,wh->hbsd", corr_x, (1-q_m) * self.read_common_mask)
            corr_k = torch.einsum("wbsd,wh->hbsd", corr_x, (1-k_m) * self.read_common_mask)
            corr_v = torch.einsum("wbsd,wh->hbsd", corr_x, (1-v_m) * self.read_common_mask)
            if U is not None:
                corr_q = corr_q @ U.t()
                corr_k = corr_k @ U.t()
                corr_v = corr_v @ U.t()
    ...
    def apply_inv_proj(self, x, w, b=None, corr_x_d=None, P=None):
        ...
        if corr_x_d is not None:
            proj = proj * out_z + corr_x_d[self.writer_idx : self.writer_idx + self.num_heads] * (1 - out_z)
        else:
            proj = proj * out_z

        if P is not None:
            proj = compress(proj, P)
    ...
    def forward(
        ...
        hidden_q, hidden_k, hidden_v, z_edges_sum = self.read(hidden_states, corr_x=corr_x, U=U)
        ...
        attn_output, z_nodes_sum = self.apply_inv_proj(attn_output, self.w_o, self.b_o, corr_x_d=corr_x_d, P=P)
...
class ErazrMLP(nn.Module):
    ...
    def read(self, x, corr_x=None, U=None):
        ...
        if corr_x is not None:
            corr_mlp = torch.einsum("wbsd,w->bsd", corr_x, (1-m) * self.read_common_mask)
            if U is not None:
    ...
    def write(self, residual, x, corr_x=None, corr_x_d=None, P=None):
        # x is (batch_size, sequence_length, hidden_size)
        # corr_x / corr_x_d is (writers, batch_size, sequence_length, hidden_size)
        # residual is (writers, batch_size, sequence_length, hidden_size)
        ...
        corr_for_node = corr_x_d if corr_x_d is not None else corr_x
        if corr_for_node is not None:
            x = x + corr_for_node[self.writer_idx] * (1 - z)
    ...
    def forward(self, hidden_states, corr_x=None, corr_x_d=None, P=None, U=None):
        residual = hidden_states

        hidden_states, z_edges_sum = self.read(hidden_states, corr_x=corr_x, U=U)
        ...
        residual, z_nodes_sum = self.write(residual, hidden_states, corr_x=corr_x, corr_x_d=corr_x_d, P=P)
...
class ErazrBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states, z_mlp_edges_sum, z_mlp_nodes_sum = self.mlp(hidden_states, corr_x=corr_x, corr_x_d=corr_x_d, P=P, U=U)
...
class ErazrModel(ErazrPretrainedModel):
    def __init__(self, config, compression_dim=0, compression_init="gaussian", freeze_projections=True):
    ...
    def write(self, tok_embeds, pos_embeds, corr_x_d=None):
        ...
        if corr_x_d is not None:
            tok_embeds = tok_embeds + corr_x_d[0] * (1 - z_tok)
        ...
        if self.compression_dim > 0:
            residual = compress(residual, self.compression_P)
    ...
    def forward(
        ...
        # --- Prepare compressed / uncompressed corr_x variants ---
        if self.compression_dim > 0 and corr_x is not None:
            corr_x_d = corr_x
            corr_x_k = corr_x @ self.compression_P.data.t()
        else:
            corr_x_d = corr_x
            corr_x_k = corr_x
        ...
        hidden_states, z_nodes_sum = self.write(inputs_embeds, position_embeds, corr_x_d=corr_x_d)
        ...
        hidden_states, z_final_edges_sum = self.read(writer_states, corr_x=corr_x_k)
...
class ErazrModelForSequenceTransformation(ErazrPretrainedModel):
    def __init__(self, config, compression_dim=0, compression_init="gaussian", freeze_projections=True):
        ...
        self.Erazr_model = ErazrModel(config, compression_dim=compression_dim, compression_init=compression_init, freeze_projections=freeze_projections)
        self.initialized = False
